...
def process_date(date):
    datafile = f"./data/splited_data_v3/period-{date}.csv"
    print(datafile)
    ...
    for advertiserNumber in range(0,48):
        ...
        budget = temp_df['budget'].iloc[0] / 20.0
        ...
        # prob += pulp.lpSum([C[i][j] * x[i][j]  for i in range(len(x)) for j in range(len(x[i]))]) <= 2.0 * CPAConstraint * z, "CPA Constraint"
        ...
        resultset[advertiserNumber] = [sumsum,pulp.value(prob.objective),sumsum / (pulp.value(prob.objective) + 1e-10),[alpha[i].varValue for i in range(48)]]
    
        with open(f"./data/pulp_v3_withoutcpa/period-{date}.txt",'w') as f:
            f.write(str(resultset))
    df.to_csv(f"./data/pulp_v3_withoutcpa/period-{date}.csv", index=False)
    del df

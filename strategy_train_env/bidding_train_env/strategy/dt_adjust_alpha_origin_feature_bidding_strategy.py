...
class DtBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self,day=0,id=0,budget=100, name="Dtaa-origin-feature-PlayerStrategy", cpa=2, category=1):
        super().__init__(budget, name, cpa, category)

        file_name = os.path.dirname(os.path.realpath(__file__))
        dir_name = os.path.dirname(file_name)
        dir_name = os.path.dirname(dir_name)
        model_path = os.path.join(dir_name, "saved_model", "DTaa_origin_feature_without7_test", "dt.pt")
        picklePath = os.path.join(dir_name, "saved_model", "DTaa_origin_feature_without7_test", "normalize_dict.pkl")

        with open(picklePath, 'rb') as f:
            normalize_dict = pickle.load(f)
        self.model = DecisionTransformerV2(state_dim=16, act_dim=1, state_mean=normalize_dict["state_mean"],

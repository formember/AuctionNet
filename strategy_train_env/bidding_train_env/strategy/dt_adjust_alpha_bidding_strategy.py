...
from bidding_train_env.baseline.dt.dt import DecisionTransformerV2
...
class DTBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self, budget=100, name="DTaa-PlayerStrategy", cpa=2, category=1):
        super().__init__(budget, name, cpa, category)

        file_name = os.path.dirname(os.path.realpath(__file__))
        dir_name = os.path.dirname(file_name)
        dir_name = os.path.dirname(dir_name)
        model_path = os.path.join(dir_name, "saved_model", "DT_Adjust_alpha", "dt.pt")
        picklePath = os.path.join(dir_name, "saved_model", "DT_Adjust_alpha", "normalize_dict.pkl")

        with open(picklePath, 'rb') as f:
            normalize_dict = pickle.load(f)
        self.model = DecisionTransformerV2(state_dim=23, act_dim=1, state_mean=normalize_dict["state_mean"],
                                         state_std=normalize_dict["state_std"],target_return=0,K=5)
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        test_state = np.array([
            ...
            self.remaining_budget / self.budget,
            np.mean(pValues),
            np.percentile(pValues, 10),
            np.percentile(pValues, 25),
            np.percentile(pValues, 40),
            np.percentile(pValues, 55),
            np.percentile(pValues, 70),
            np.percentile(pValues, 85),
            historical_bid_mean,
            last_three_bid_mean,
        ...
        alpha = self.model.take_actions(test_state,
                                        pre_reward=sum(history_conversion[-1]) if len(history_conversion) != 0 else None,target_return=np.array([self.cpa/130.0,self.budget/4850.0]))
        bids = alpha * pValues
        print(alpha)
        return bids
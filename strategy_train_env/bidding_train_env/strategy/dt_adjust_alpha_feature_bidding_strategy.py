...
class DTBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self, day=0,id=0,budget=100, name="DTaa-feature-v2-PlayerStrategy", cpa=2, category=1):
        super().__init__(budget, name, cpa, category)

        file_name = os.path.dirname(os.path.realpath(__file__))
        dir_name = os.path.dirname(file_name)
        dir_name = os.path.dirname(dir_name)
        model_path = os.path.join(dir_name, "saved_model", "DT_Adjust_alpha_feature_without7_v2", "dt.pt")
        picklePath = os.path.join(dir_name, "saved_model", "DT_Adjust_alpha_feature_without7_v2", "normalize_dict.pkl")

        with open(picklePath, 'rb') as f:
            normalize_dict = pickle.load(f)
        self.model = DecisionTransformerV2(state_dim=49, act_dim=1, state_mean=normalize_dict["state_mean"],
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        alpha = self.model.take_actions(test_state,
                                        pre_reward=sum(history_conversion[-1]) if len(history_conversion) != 0 else None,target_return=np.array([self.remaining_budget / self.budget]))
        bids = alpha * pValues
        print(alpha)
        return bids
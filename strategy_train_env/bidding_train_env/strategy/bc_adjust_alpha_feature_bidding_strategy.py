...
class BcBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self,day=0,id=0, budget=100, name="Bcaa-feature-PlayerStrategy", cpa=2, category=1):
        super().__init__(budget, name, cpa, category)

        file_name = os.path.dirname(os.path.realpath(__file__))
        dir_name = os.path.dirname(file_name)
        dir_name = os.path.dirname(dir_name)
        model_path = os.path.join(dir_name, "saved_model", "BC_adjust_alpha_feature_without7", "bc_model.pth")
        dict_path = os.path.join(dir_name, "saved_model", "BC_adjust_alpha_feature_without7", "normalize_dict.pkl")

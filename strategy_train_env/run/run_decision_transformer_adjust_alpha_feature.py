...
def train_model():
    ...
    replay_buffer = EpisodeConstraintReplayBuffer(state_dim, 1, "./data/adjust_best_alpha/training_data_dt_feature_without7.csv",K=k)
    save_normalize_dict({"state_mean": replay_buffer.state_mean, "state_std": replay_buffer.state_std},
                        "saved_model/DT_Adjust_alpha_feature_without7_v2")
    logger.info(f"Replay buffer size: {len(replay_buffer.trajectories)}")
    step_num = 10000
    model = DecisionTransformerV2(state_dim=state_dim, act_dim=1, state_mean=replay_buffer.state_mean,total_steps=step_num,
                                state_std=replay_buffer.state_std,K=k)
    ...
    for states, actions, rewards, dones, rtg, c_b, timesteps, attention_mask in dataloader:
        ...
        if np.mean(train_loss) < min_loss:
            ...
            model.save_net("saved_model/DT_Adjust_alpha_feature_without7_v2")
        logger.info(f"Step: {i} Action loss: {np.mean(train_loss)}")
        model.scheduler.step()

    model.save_net("saved_model/DT_Adjust_alpha_feature_without7_v2")
    test_state = np.ones(state_dim, dtype=np.float32)
    logger.info(f"Test action: {model.take_actions(test_state,target_return=np.array([0.5]))}")
    logger.info(f"Test action: {model.take_actions(test_state,target_return=np.array([0.5]),pre_reward=0)}")
    logger.info(f"Test action: {model.take_actions(test_state,target_return=np.array([0.5]),pre_reward=0)}")

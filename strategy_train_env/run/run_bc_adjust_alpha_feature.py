...
def train_model():
    ...
    train_data_path = "./data/adjust_best_alpha/training_data_dt_feature_without7.csv"
    ...
    step_num = 10000
    model = BC(dim_obs=state_dim,hidden_size=256,total_training_steps =step_num,actor_lr=0.001)
    
    batch_size = 100
    for i in range(step_num): 
        states, actions, _, _, _ = replay_buffer.sample(batch_size)
        
        # Validate states and actions to ensure no NaN or Inf values
        if np.isnan(states).any() or np.isinf(states).any():
            logger.error(f"Invalid values detected in states at step {i}. Skipping this batch.")
            continue
        if np.isnan(actions).any() or np.isinf(actions).any():
            logger.error(f"Invalid values detected in actions at step {i}. Skipping this batch.")
            continue
    ...
    model.save_jit("saved_model/BC_adjust_alpha_feature_without7")
...
def load_model():
    ...
    """
    model = BC(dim_obs=16)
    model.load_net("saved_model/BC_adjust_alpha_feature_without7")

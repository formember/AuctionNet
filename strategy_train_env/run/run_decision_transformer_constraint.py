...
def train_model():
    ...
    replay_buffer = EpisodeConstraintReplayBuffer(16, 1, "./data/subopt_rl_data/training_data-rlData_fill_action.csv",K=k)
    save_normalize_dict({"state_mean": replay_buffer.state_mean, "state_std": replay_buffer.state_std},
                        "saved_model/DT_constraint_test")
    logger.info(f"Replay buffer size: {len(replay_buffer.trajectories)}")

    model = DecisionTransformerConstraint(state_dim=state_dim, act_dim=1, state_mean=replay_buffer.state_mean,
    ...
    for states, actions, rewards, dones, rtg, c_b, timesteps, attention_mask in dataloader:
        ...
        if np.mean(train_loss) < min_loss:
            ...
            model.save_net("saved_model/DT_constraint_test")

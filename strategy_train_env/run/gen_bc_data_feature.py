        ...
        for advertiserNumber in range(0, 48):
            ...
            for repeat in range(10):
                ...
                for timeStepIndex in range(0, 48):
                    ...
                    adjust_alpha = max(0,best_alpha + cpa * (np.random.rand() - 0.5)*2)
                    # print(timeStepIndex, best_alpha, adjust_alpha,remaining_budget)
                    ...
                    historyImpressionResult.append(np.array([(tick_conversion[i], tick_conversion[i]) for i in range(now_pValues.shape[0])]))
...
def main():
    ...
    training_data.to_csv("./data/adjust_best_alpha/training_data_feature_cpa.csv", index=False)

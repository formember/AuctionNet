...
data = pd.read_csv("./linear_solution/alpha_cpa_time.csv")
train_data = data[(data["alpha"] < 300) & (data["budget"] % 10 ==0) & (data["alpha"] != 100)]  # Filter out rows where time is less than 3000

input_data = torch.tensor(train_data["time"].values, dtype=torch.float32).view(-1, 1)
budget = torch.tensor(train_data["budget"].values, dtype=torch.float32).view(-1, 1)
budget = budget / 3000.0  # Normalize budget to [0, 1]

labels = torch.tensor(train_data["alpha"].values, dtype=torch.float32).view(-1, 1)
labels = labels / 300.0  # Normalize labels to [0, 1]
eval_data = data

eval_input_data = torch.tensor(eval_data["time"].values, dtype=torch.float32).view(-1, 1)
eval_budget = torch.tensor(eval_data["budget"].values, dtype=torch.float32).view(-1, 1)
eval_budget = eval_budget / 3000.0  # Normalize budget to [0, 1]
...
plt.savefig("all_advertisers_Bspline_train_time.png")
plt.close()
...
plt.tight_layout()  # Adjust layout to prevent overlap
plt.savefig("all_advertisers_Bspline_few_time.png")
plt.close()

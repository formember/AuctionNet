...
class SimpleNet(nn.Module):
    ...
    def forward(self, x , budget):
        ...
        k = k.view(-1, 1)
        b = b.view(-1, 1)
        return torch.add(torch.mul(k, budget),b),k,b
...
for epoch in range(epochs):
    for batch in train_loader:
        ...
        # 前向传播
        outputs,_,_ = model(input_data, budget)
...
for eval_batch in eval_loader:
    eval_input_data, eval_budget, eval_labels = eval_batch
    with torch.no_grad():
        eval_outputs, k, b = model(eval_input_data, eval_budget)
...
plt.savefig("all_advertisers.png")
plt.close()

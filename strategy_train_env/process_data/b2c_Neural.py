...
class SimpleNet(nn.Module):
    def __init__(self):
        ...
        self.id_linear = nn.Linear(8, 4)  # Linear layer for embedding
    ...
    def forward(self, x , budget):
        ...
        emb = self.relu(self.id_linear(emb))  # Apply linear transformation to embedding
        budget = budget.view(-1, 1)
        emb = emb.view(-1, 4)  # Ensure emb is of shape (batch_size, 8)
        emb = torch.cat((emb, budget), dim=1)  # Concatenate budget to the embedding
        k = self.relu(self.linear1(emb))
        k = self.relu(self.linear2(k))
        k = self.linear3(k)
        return k
...
plt.savefig("all_advertisers_neural_train.png")
plt.close()
...
plt.tight_layout()  # Adjust layout to prevent overlap
plt.savefig("all_advertisers_neural_few.png")
plt.close()
